...
template <class T, class Mode> struct gemm<gemm_policy<T, T, T, T, T, Mode>> {
  inline culina::status_t
  operator()(culina::handle_base *const handle, const culina::blas::op_t op_a,
             const culina::blas::op_t op_b, const std::size_t m,
             const std::size_t n, const std::size_t k, const T alpha,
             const T *a_ptr, const std::size_t lda, const T *b_ptr,
    ...
    cublasGemmEx(handle->cublas_handle(), culina::blas::to_cublas_op_t(op_a),
                 ...
                 culina::cuda_data_type<T>, lda, b_ptr,
                 culina::cuda_data_type<T>, ldb, &beta, c_ptr,
                 culina::cuda_data_type<T>, ldc,
