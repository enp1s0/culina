...
__global__ void generic_gemv_kernel(const culina::blas::op_t op_a,
                                    ...
                                    const std::size_t lda, const Tx *x_ptr,
                                    const std::size_t incx, const Ts beta,
...
template <class Ta_, class Tx_, class Ty_, class Ts_, class ComputeT_,
          class Mode_>
struct gemv_policy {
  using Ta = Ta_;
  using Tx = Tx_;
...
template <class GemvPolicy> struct gemv {
  ...
  operator()(culina::handle_base *const handle, const culina::blas::op_t op_a,
    ...
    const std::size_t grid_size =
        ((op_a == culina::blas::op_t::N ? m : n) * warp_size + block_size - 1) /
        block_size;
...
template <> struct gemv<default_gemv_policy<float, culina::default_mode>> {
  inline culina::status_t
  operator()(culina::handle_base *const handle, const culina::blas::op_t op_a,
             const std::size_t m, const std::size_t n, const float alpha,
             const float *a_ptr, const std::size_t lda, const float *x_ptr,
             const std::size_t incx, const float beta, float *const y_ptr,
             const std::size_t incy) {
    cublasSgemv(handle->cublas_handle(), culina::blas::to_cublas_op_t(op_a), m,
                n, &alpha, a_ptr, lda, x_ptr, incx, &beta, y_ptr, incy);
    return culina::status_t::success;
  }
};

template <> struct gemv<default_gemv_policy<double, culina::default_mode>> {
  inline culina::status_t
  operator()(culina::handle_base *const handle, const culina::blas::op_t op_a,
             const std::size_t m, const std::size_t n, const double alpha,
             const double *a_ptr, const std::size_t lda, const double *x_ptr,
             const std::size_t incx, const double beta, double *const y_ptr,
             const std::size_t incy) {
    cublasDgemv(handle->cublas_handle(), culina::blas::to_cublas_op_t(op_a), m,
                n, &alpha, a_ptr, lda, x_ptr, incx, &beta, y_ptr, incy);
    return culina::status_t::success;
  }
};
