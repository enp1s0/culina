...
__global__ void
generic_dot_kernel(Tz *const result_ptr, const Tx *const x_ptr,
...
template <class DotPolicy> struct dot {
  ...
  operator()(culina::handle_base *const handle, const std::size_t n,
    ...
    generic_dot_kernel<Tx, Ty, Tz>
        <<<grid_size, block_size, 0, handle->stream()>>>(c_ptr, x_ptr, incx,
                                                         y_ptr, incy, n);

    CULINA_CHECK_ERROR(cudaMemcpyAsync(z_ptr, c_ptr, sizeof(Tz),
