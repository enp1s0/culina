...
#define CULINA_CHECK_ERROR(status)                                             \
  culina::detail::check_error(status, __FILE__, __LINE__, __func__)
#define CULINA_CHECK_ERROR_M(status, message)                                  \
  culina::detail::check_error(status, __FILE__, __LINE__, __func__, (message))

namespace culina::detail {
inline void check_error(cudaError_t error, const std::string filename,
                        const std::size_t line, const std::string funcname,
...
inline void check_error(cublasStatus_t error, const std::string filename,
                        const std::size_t line, const std::string funcname,
                        const std::string message = "") {
  if (error != CUBLAS_STATUS_SUCCESS) {
    std::string error_string;
#define CUBLAS_ERROR_CASE(c)                                                   \
  case c:                                                                      \
    error_string = #c;                                                         \
    break
    switch (error) {
