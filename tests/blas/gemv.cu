...
void eval(
  ...
  for (std::size_t i = 0; i < num_vec_X_elms; i++) {vec_X.get()[i] = static_cast<T>(dist(mt));}
  for (std::size_t i = 0; i < num_vec_Y_elms; i++) {vec_Y.get()[i] = static_cast<T>(dist(mt));}
...
#pragma omp parallel for
  for (std::size_t im = 0; im < vec_Y_len; im++) {
    ...
    for (std::size_t in = 0; in < vec_X_len; in++) {
      const double a = mat_A.get()[op_A == culina::blas::op_t::N ? im + in * lda : in + im * lda];
      const double x = vec_X.get()[in * incx];
      c += a * x;
    }
    vec_Z.get()[im] = c * static_cast<double>(alpha) + static_cast<double>(vec_Y.get()[im * incy]) * static_cast<double>(beta);
  }
